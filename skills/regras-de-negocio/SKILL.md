...
3. Um teste por cláusula EARS, com o ID no nome (`RN_ORD_012_…`), escrito **uma vez a partir da spec** — pelo `test-designer` ou
   em sessão separada. Ele falha até a regra existir. É um sensor, não red-green por micro-passo: TDD forçado no loop do agente deu
   "no clearly discernable difference" de qualidade a 3–8× o custo em tokens (Böckeler, Thoughtworks, ago/2026 —
   `docs/referencias.md`).
...
5. Atualize no bloco da regra: `Código:`, `Teste:`, `Confiança:` (`verified` = tem teste que passa; `inferred` = lida do código, sem
   teste — esta é a definição; os `docs/regras/` apontam para cá), `Última revisão:`, `Dono:`.
