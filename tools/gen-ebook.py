...
<p>Isto <em>não</em> é TDD forçado no loop do agente — prática que a Thoughtworks mediu sem diferença discernível de qualidade e com 3–8× de custo em tokens (Böckeler, 2026 — <code>docs/referencias.md</code> do kit), e que o roteiro desaconselha. É outra coisa: um teste por cláusula EARS, escrito <b>uma vez</b> a partir da spec pelo <code>test-designer</code> (ou numa sessão separada), que fica vermelho até a regra existir e depois vira sensor permanente. Quem implementa não edita esses testes; faz passar.</p>
