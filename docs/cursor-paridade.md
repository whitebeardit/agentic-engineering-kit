...
| Bloquear **escrita** em segredo/migration/baseline | `PreToolUse` Edit\|Write → exit 2 (antes de escrever) | `preToolUse` (matcher `Write\|Edit\|…`) → `{"permission":"deny"}`, **antes** de escrever — doc do Cursor lida em 2026-09-09 (o evento genérico `preToolUse` não existia na leitura de 30/08) | `afterFileEdit` reverte e registra (defesa em profundidade, não mais o único mecanismo) |
