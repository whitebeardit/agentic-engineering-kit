...
- Nomes no plugin: skills `/kit:card-intake`, `/kit:run-and-test`, `/kit:regras-de-negocio`; agentes `kit:impact-analyzer`, `kit:legacy-navigator`, `kit:test-designer`, `kit:dotnet-reviewer`, `kit:contract-reviewer`; spec-driven `/tlc:tlc-spec-driven`. Instalação standalone (`apply.sh --standalone`) usa os nomes sem prefixo.
...
- Hooks de enforcement (protect-paths, guard-bash, dotnet-format) vêm de `.claude/settings.json` deste repo, não do plugin.
