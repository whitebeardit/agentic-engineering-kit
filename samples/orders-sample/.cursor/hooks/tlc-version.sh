...
# 1) cópias locais (ordem de preferência)
found=(); vers=()
for f in "$PWD/.claude/skills/tlc-spec-driven/SKILL.md" \
         "$HOME"/.claude/plugins/cache/*/tlc/*/SKILL.md "$HOME"/.claude/plugins/cache/*/tlc/SKILL.md \
         "$HOME/.claude/skills/tlc-spec-driven/SKILL.md" \
         "$PWD/.cursor/skills/tlc-spec-driven/SKILL.md" "$HOME/.cursor/skills/tlc-spec-driven/SKILL.md"; do
  [ -f "$f" ] && { found+=("$f"); vers+=("$(ver_of "$f")"); }
done
...
if [ ${#found[@]} -eq 0 ]; then
  ...
  local_v="${vers[0]}"
  if [ ${#found[@]} -gt 1 ]; then msg="tlc-spec-driven duplicado (${#found[@]} cópias: ${found[*]}). Mantenha só a do plugin; remova ~/.claude/skills/tlc-spec-driven. "; fi
