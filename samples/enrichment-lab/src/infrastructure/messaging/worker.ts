...
export type Resultado =
  'gravado' | 'sem-mudanca' | 'duplicado' | 'descartado' | 'retry';
...
export class EventoIngestaoWorker {
  constructor(
    ...
    private readonly limiarN: number,
  ...
  private async processar(
    ...
    try {
      ...
      const resultado = await this.gravarComRetry(guarda.evento);
      await this.repo.registrarProcessado(evento.eventId);
      await this.fila.confirmar(id);
      this.log.info(
        resultado === 'gravado' ? 'evento gravado' : 'evento sem mudança',
        ctx,
      );
      return resultado;
    } catch (erro) {
  ...
  private async gravarComRetry(
    ...
    for (let tentativa = 1; ; tentativa++) {
      ...
      const { changed, eventos } = cliente.aplicar(evento, this.limiarN);
      ...
      for (const e of eventos) await this.publicador.publicar(e);
      return 'gravado';
    }
  }
}
