...
- Testes: `npm test` (≈ 3 s; 28 testes: 14 regra RN_ENR_*, 3 arquitetura, 1 characterization, 10 integração)
...
- Ordem das guardas é fixa (`docs/regras/enriquecimento.md`): idempotência → dígitos → blacklist → (apto e limiar, RN-ENR-004 pendente) → gravação condicional.
